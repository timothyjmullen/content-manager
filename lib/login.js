...
auth.createUserWithEmailAndPassword(em, pw).catch(function(error) {
  if (error) {
    switch (error.code) {
      ...
      case "auth/invalid-email":
        ...
        break;
      default:
        document.getElementById("alert").innerHTML = "<span class=\"glyphicon glyphicon-warning-sign\" aria-hidden=\"true\"></span> " + error;
        document.getElementById("alert").className = "alert alert-warning";
    }
  }
});
...
function login(email, pass) {
  ...
  firebase.auth().signInWithEmailAndPassword(email || em, pass || pw).catch(function(error) {
    // Handle Errors here.
    ...
    if (errorCode) {
      switch (errorCode) {
        ...
        case "auth/wrong-password":
          ...
          break;
        default:
          document.getElementById("alert").innerHTML = "<span class=\"glyphicon glyphicon-warning-sign\" aria-hidden=\"true\"></span> " + errorMessage;
          document.getElementById("alert").className = "alert alert-warning";
      }
    }
  });
...
auth.onAuthStateChanged(function(user) {
  if (user) {
    ...
    name = user.displayName || document.getElementById("txtName").value;
    email = user.email;
    ...
    firebase.database().ref("users/" + uid).once('value').then(function(snap) {
      if (snap.val()) {
        ...
        /* If user does not exist, update profile and user database */
        user.updateProfile({
          displayName: name,
          photoURL: 'https://www.gravatar.com/avatar/' + calcMD5(email.replace(/^[\s]*|[\s]*$/g, '').toLowerCase()) + '?d=retro'
        });
        var updates = {};
        updates["users/" + uid] = {
          name: name,
        ...
        updates["users/email-map/" + btoa(email)] = uid;
...
function regorlogSwap() {
  ...
  if (/Register/.test(btn.innerHTML)) {
    ...
    document.getElementById("log-head").innerHTML = 'Register';
    ...
    document.getElementById("log-head").innerHTML = 'Login';
    document.getElementById("alert").className = "alert";
  }
}
